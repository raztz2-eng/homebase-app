...
const ST = {
  ...
  textarea:{background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.12)",borderRadius:10,padding:"9px 12px",color:"#e8eaf0",fontSize:14,width:"100%",outline:"none",boxSizing:"border-box",resize:"vertical",minHeight:80,fontFamily:"inherit"},
...
export default function Kelly({ lang, onAddTask }) {
  ...
  // Medical form - updated with visitDesc + treatment + add to vet tasks
  const mf0 = {date:"",type:"בדיקה",visitDesc:"",treatment:"",treatFreq:2,treatWeeks:2,treatDays:[],vet:""};
  ...
  const [treatAdded, setTreatAdded] = useState({});
  ...
  useEffect(()=>{
    const subs = [
      ...
      listenCol("kelly_settings", (d)=>{
        const insDoc=d.find(x=>x.id==="insurance"); if(insDoc) setIns(insDoc);
        const foodDoc=d.find(x=>x.id==="food"); if(foodDoc) setFood(foodDoc);
        const regDoc=d.find(x=>x.id==="register"); if(regDoc) setReg(regDoc);
        setLoading(false);
      }),
    ];
  ...
  const saveWeight=async()=>{
    ...
    setEditWId(null); setWf({date:new Date().toISOString().slice(0,10),kg:"",note:""});
  };
  ...
  const toggleTreatDay=(d)=>setMf(p=>({...p,treatDays:p.treatDays.includes(d)?p.treatDays.filter(x=>x!==d):[...p.treatDays,d]}));
  ...
  const saveVac=async()=>{
    ...
    setEditVacId(null); setShowVacF(false); setVacF({name:"",last:"",next:"",notes:""});
  };
  ...
  // Medical - save with new fields
  const saveMed=async()=>{
    if(!mf.visitDesc&&!mf.treatment) return;
    const id=editMId||nid();
    await saveDoc("kelly_medical",id,{...mf,id});
    setEditMId(null); setShowMf(false); setMf(mf0); setShowTreatForm(false);
  };
  const startEditMed=(r)=>{
    setEditMId(r.id);
    setMf({
      date:r.date||"",type:r.type||"בדיקה",
      visitDesc:r.visitDesc||r.desc||"",
      treatment:r.treatment||"",
      treatFreq:r.treatFreq||2,treatWeeks:r.treatWeeks||2,
      treatDays:r.treatDays||[],vet:r.vet||""
    });
    if(r.treatment) setShowTreatForm(true);
    setShowMf(true);
  };
  const deleteMed=async(id)=>deleteDocById("kelly_medical",id);

  // Add treatment from medical record to vet tasks
  const addTreatmentToVetTasks=async(rec)=>{
    if(!rec.treatment) return;
    const id=nid();
    const taskData={
      id,instruction:rec.treatment,
      freq:rec.treatFreq||2,weeks:rec.treatWeeks||2,
      start:rec.date||new Date().toISOString().slice(0,10),
      days:rec.treatDays||[],note:"מביקור: "+rec.date,
    };
    await saveDoc("kelly_tasks",id,taskData);
    if(onAddTask) onAddTask({id:nid(),he:rec.treatment,en:rec.treatment,cat:"other",priority:"medium",person:"Both",recur:"weekly",dueDate:"",note:"טיפול קלי",done:false,lastDone:""});
    setTreatAdded(p=>({...p,[rec.id]:true}));
    setTimeout(()=>setTreatAdded(p=>({...p,[rec.id]:false})),2000);
  };
    ...
    try {
      ...
      const resp=await fetch("https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key="+GEMINI_KEY,{
        ...
        body:JSON.stringify({contents:[{parts:[
          {text:"אתה וטרינר מומחה. נתח את המסמך הרפואי של הכלב. תן סיכום בעברית: 1) ממצאים עיקריים 2) אבחנות 3) תרופות/טיפולים שנרשמו 4) המלצות להמשך. פורמט ברור."},
      ...
      const id=nid();
      await saveDoc("kelly_medical",id,{id,date:new Date().toISOString().slice(0,10),type:"בדיקה",visitDesc:"ניתוח AI: "+text.slice(0,200),treatment:"",vet:"Gemini AI"});
  ...
  const saveClaim=async()=>{
    ...
    setEditCId(null); setShowCf(false); setCf({date:"",amount:"",reason:"",status:"הוגש"});
  };
  ...
  // Food
  const saveFood=async(key,val)=>{const updated={...food,[key]:val,id:"food"};setFood(updated);await saveDoc("kelly_settings","food",updated);};
  const addFoodTask=()=>{
    ...
    setFoodTaskAdded(true); setTimeout(()=>setFoodTaskAdded(false),3000);
  };

  // Register
  const saveReg=async(key,val)=>{const updated={...reg,[key]:val,id:"register"};setReg(updated);await saveDoc("kelly_settings","register",updated);};
      ...
      <div style={{padding:"20px 22px",maxWidth:860,margin:"0 auto"}}>
        ...
        {sec==="tasks"&&(
          <div>
            ...
            {vetTasks.length===0&&<div style={{...ST.card,textAlign:"center",padding:30,color:"#6b7280"}}>אין משימות טיפול עדיין 🐕</div>}
          </div>
        )}
        ...
        {sec==="medical"&&(
          <div>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18}}>
              ...
              <button onClick={()=>{setShowMf(!showMf);setEditMId(null);setMf(mf0);setShowTreatForm(false);}} style={ST.btn}>+ הוסף רשומה</button>
            </div>
            ...
            <div style={{...ST.card,marginBottom:18,borderColor:"rgba(16,185,129,0.2)"}}>
              ...
              {analysisResult&&(
                <div style={{marginTop:14,padding:14,background:"rgba(16,185,129,0.06)",border:"1px solid rgba(16,185,129,0.2)",borderRadius:10,fontSize:13,lineHeight:1.7,whiteSpace:"pre-wrap"}}>
                  <div style={{fontSize:11,color:"#10b981",fontWeight:700,marginBottom:8}}>✅ ניתוח הושלם — נוסף לתיק הרפואי</div>
                  {analysisResult}
                </div>
              )}
            </div>
            ...
            {showMf&&(
              <div style={{...ST.card,marginBottom:18,borderColor:"rgba(99,102,241,0.3)"}}>
                <div style={{fontSize:14,fontWeight:700,color:"#a5b4fc",marginBottom:14}}>{editMId?"✏️ עריכת רשומה":"📋 רשומה חדשה"}</div>

                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginBottom:12}}>
                  <div><div style={{fontSize:11,color:"#6b7280",marginBottom:4}}>תאריך ביקור</div><input type="date" value={mf.date} onChange={e=>setMf({...mf,date:e.target.value})} style={ST.inp}/></div>
                  <div><div style={{fontSize:11,color:"#6b7280",marginBottom:4}}>סוג ביקור</div>
                    <select value={mf.type} onChange={e=>setMf({...mf,type:e.target.value})} style={{...ST.inp,appearance:"none"}}>
                      {["בדיקה","ניתוח","טיפול","חיסון","אחר"].map(t=><option key={t}>{t}</option>)}
                    </select>
                  </div>
                  <div><div style={{fontSize:11,color:"#6b7280",marginBottom:4}}>שם וטרינר</div><input value={mf.vet} onChange={e=>setMf({...mf,vet:e.target.value})} placeholder="ד.ר כהן..." style={ST.inp}/></div>
                </div>

                {/* תיאור ביקור */}
                <div style={{marginBottom:12}}>
                  <div style={{fontSize:12,fontWeight:600,color:"#a5b4fc",marginBottom:6}}>📋 תיאור הביקור</div>
                  <textarea
                    value={mf.visitDesc}
                    onChange={e=>setMf({...mf,visitDesc:e.target.value})}
                    placeholder="תאר מה קרה בביקור — תסמינים, ממצאים, אבחנה..."
                    style={ST.textarea}
                  />
                </div>

                {/* טיפול */}
                <div style={{marginBottom:12,padding:14,background:"rgba(99,102,241,0.05)",border:"1px solid rgba(99,102,241,0.15)",borderRadius:12}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                    <div style={{fontSize:12,fontWeight:600,color:"#a5b4fc"}}>💊 טיפול / הנחיות</div>
                    <button onClick={()=>setShowTreatForm(!showTreatForm)} style={{background:"none",border:"1px solid rgba(99,102,241,0.3)",borderRadius:8,padding:"3px 10px",color:"#a5b4fc",fontSize:11,cursor:"pointer"}}>
                      {showTreatForm?"הסתר פרטים ▲":"הוסף פרטי טיפול ▼"}
                    </button>
                  </div>
                  <textarea
                    value={mf.treatment}
                    onChange={e=>setMf({...mf,treatment:e.target.value})}
                    placeholder="תאר את הטיפול — תרופות, מינון, הנחיות וטרינר..."
                    style={{...ST.textarea,minHeight:60}}
                  />
                  {showTreatForm&&mf.treatment&&(
                    <div style={{marginTop:10}}>
                      <div style={{fontSize:11,color:"#6b7280",marginBottom:8}}>📅 לוח זמנים לטיפול (לצורך הוספה למשימות)</div>
                      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10}}>
                        <div><div style={{fontSize:11,color:"#6b7280",marginBottom:4}}>פעמים בשבוע</div><input type="number" min="1" max="7" value={mf.treatFreq} onChange={e=>setMf({...mf,treatFreq:parseInt(e.target.value)||1})} style={ST.inp}/></div>
                        <div><div style={{fontSize:11,color:"#6b7280",marginBottom:4}}>משך בשבועות</div><input type="number" min="1" value={mf.treatWeeks} onChange={e=>setMf({...mf,treatWeeks:parseInt(e.target.value)||1})} style={ST.inp}/></div>
                      </div>
                      <div style={{fontSize:11,color:"#6b7280",marginBottom:8}}>ימי הטיפול:</div>
                      <div style={{display:"flex",gap:8}}>
                        {DAYS_HE.map((d,i)=><button key={i} onClick={()=>toggleTreatDay(i)} style={{width:34,height:34,borderRadius:"50%",border:"none",background:mf.treatDays.includes(i)?"#6366f1":"rgba(255,255,255,0.06)",color:mf.treatDays.includes(i)?"#fff":"#6b7280",fontWeight:700,fontSize:11,cursor:"pointer"}}>{d}</button>)}
                      </div>
                    </div>
                  )}
                </div>
                ...
                <div style={{display:"flex",gap:8}}>
                  ...
                  {editMId&&<button onClick={()=>{setEditMId(null);setShowMf(false);setShowTreatForm(false);}} style={{...ST.btn,background:"rgba(255,255,255,0.08)"}}>ביטול</button>}
                </div>
              </div>
            )}
            ...
            {medRecs.map(r=>{
              const tc={בדיקה:"#6366f1",ניתוח:"#ef4444",טיפול:"#10b981",חיסון:"#06b6d4",אחר:"#6b7280"};
              const em={ניתוח:"🔪",טיפול:"💊",חיסון:"💉",בדיקה:"🩺"};
              return(
                <div key={r.id} style={{...ST.card,marginBottom:12}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
                    <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
                      <span style={{fontSize:20}}>{em[r.type]||"🩺"}</span>
                  ...
                  {(r.visitDesc||r.desc)&&(
                    <div style={{marginBottom:8}}>
                      <div style={{fontSize:11,color:"#6b7280",marginBottom:4,fontWeight:600}}>📋 תיאור ביקור:</div>
                      <div style={{fontSize:13,lineHeight:1.6,color:"#d1d5db"}}>{r.visitDesc||r.desc}</div>
                    </div>
                  )}

                  {/* Treatment */}
                  {r.treatment&&(
                    <div style={{padding:10,background:"rgba(99,102,241,0.06)",border:"1px solid rgba(99,102,241,0.15)",borderRadius:10,marginBottom:8}}>
                      <div style={{fontSize:11,color:"#a5b4fc",marginBottom:4,fontWeight:600}}>💊 טיפול:</div>
                      <div style={{fontSize:13,lineHeight:1.6,color:"#d1d5db",marginBottom:8}}>{r.treatment}</div>
                      {(r.treatFreq||r.treatDays?.length>0)&&(
                        <div style={{fontSize:11,color:"#6b7280",marginBottom:8}}>
                          {r.treatFreq&&<span>{r.treatFreq}x/שבוע · </span>}
                          {r.treatWeeks&&<span>{r.treatWeeks} שבועות · </span>}
                          {r.treatDays?.length>0&&<span>ימים: {r.treatDays.map(i=>DAYS_HE[i]).join(" ")}</span>}
                        </div>
                      )}
                      <button
                        onClick={()=>addTreatmentToVetTasks(r)}
                        style={{background:treatAdded[r.id]?"rgba(16,185,129,0.15)":"rgba(99,102,241,0.15)",border:"1px solid "+(treatAdded[r.id]?"rgba(16,185,129,0.3)":"rgba(99,102,241,0.3)"),borderRadius:8,padding:"5px 12px",color:treatAdded[r.id]?"#6ee7b7":"#a5b4fc",fontSize:12,cursor:"pointer",fontWeight:600}}
                      >
                        {treatAdded[r.id]?"✓ נוסף למשימות טיפול!":"📋 הוסף למשימות טיפול"}
                      </button>
                    </div>
                  )}
                </div>
