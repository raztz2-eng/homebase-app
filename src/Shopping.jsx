...
const PRICES={
  "חלב":7,"ביצים":18,"לחם":10,"עגבניות":8,"מלפפון":7,
  "בצל":5,"גזר":6,"תפוחים":14,"בננות":9,"תפוזים":8,
  "אבוקדו":18,"ברוקולי":12,"חסה":8,"פלפל":14,
  "גבינה לבנה":9,"גבינה":35,"יוגורט":5,"חמאה":22,"קוטג":6,
  "עוף":25,"חזה עוף":38,"בשר טחון":45,"סלמון":95,"טונה":16,
  "שמן זית":28,"שמן":18,"סוכר":8,"קמח":7,"אורז":14,"פסטה":8,
  "קפה":55,"שוקולד":15,"נייר טואלט":35,"אבקת כביסה":45,
  "שמפו":28,"סבון":12,"משחת שיניים":14,
  "milk":7,"eggs":18,"bread":10,"tomatoes":8,"cucumber":7,
  "apples":14,"bananas":9,"cheese":35,"chicken":25,"rice":14,
  "pasta":8,"coffee":55,"shampoo":28,"soap":12,
};
...
export default function Shopping({lang,theme}){
  ...
  const totalEst=items.filter(i=>!i.done).reduce((s,i)=>{
    const avg=getAvg(i.he);return s+(avg||0)*(i.qty||1);
  },0);
  ...
  const addItem=async()=>{
    ...
    const id=newId();
    await saveDoc(COL.shopping,id,{id,he:form.he.trim(),en:form.he.trim(),cat:form.cat,qty:parseFloat(form.qty)||1,unit:form.unit,note:form.note,done:false,_order:Date.now()});
    setForm({he:"",cat:"produce",qty:"1",unit:"",note:""});
    setShowForm(false);
  };
  const addFav=async(fav)=>{
    const id=newId();
    await saveDoc(COL.shopping,id,{id,he:fav.he,en:fav.en,cat:fav.cat,qty:1,unit:"",note:"",done:false,_order:Date.now()});
      ...
      <div style={{display:"flex",alignItems:"center",gap:10,padding:"13px 16px",borderRadius:12,background:it.done?"rgba(16,185,129,0.05)":TH.input,border:it.done?"1px solid rgba(16,185,129,0.15)":"1px solid "+TH.cardBorder,marginBottom:8}}>
        ...
        <div style={{flex:1,textAlign:isRTL?"right":"left"}}>
          <div style={{fontSize:15,fontWeight:600,textDecoration:it.done?"line-through":"none",color:it.done?TH.mutedText:TH.text}}>
            {it.he}{it.qty>1&&<span style={{fontSize:12,color:TH.subText,marginRight:6}}> x{it.qty}{it.unit?" "+it.unit:""}</span>}
          </div>
        ...
        <button onClick={()=>remove(it.id)} style={{background:"none",border:"none",color:TH.mutedText,cursor:"pointer",fontSize:18,padding:"0 4px",flexShrink:0}}>x</button>
      </div>
    );
  };
  ...
  return(
    <div style={{fontFamily:"'Outfit',sans-serif",color:TH.text,direction:isRTL?"rtl":"ltr",minHeight:"100vh",background:TH.bg,transition:"background .3s,color .3s"}}>
      ...
      <div style={{padding:"16px 18px",maxWidth:800,margin:"0 auto"}}>
        <button onClick={()=>setShowForm(!showForm)} style={{width:"100%",padding:"16px",marginBottom:16,background:showForm?"rgba(99,102,241,0.15)":"linear-gradient(135deg,#6366f1,#06b6d4)",border:showForm?"2px solid rgba(99,102,241,0.4)":"none",borderRadius:16,color:"#fff",fontSize:17,fontWeight:700,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:10,boxShadow:showForm?"none":"0 4px 20px rgba(99,102,241,0.3)"}}>
          <span style={{fontSize:26}}>{showForm?"x":"+"}</span>
        ...
        {showForm&&(
          <div style={{...S.card,marginBottom:16,borderColor:"rgba(99,102,241,0.3)"}}>
            <div style={{marginBottom:10}}>
              ...
              <input value={form.he} onChange={e=>setForm({...form,he:e.target.value})} onKeyDown={e=>e.key==="Enter"&&addItem()} placeholder={isRTL?"לדוגמה: תפוחים, חלב...":"e.g. Apples, Milk..."} style={{...S.inp,fontSize:16,padding:"12px 14px"}} autoFocus/>
              {form.he&&getAvg(form.he)&&<div style={{fontSize:12,color:"#10b981",marginTop:5}}>💡 {isRTL?"מחיר שוק":"Avg"}: <strong>₪{getAvg(form.he)}</strong></div>}
            </div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 2fr",gap:8,marginBottom:10}}>
              ...
              <div><div style={{fontSize:11,color:TH.subText,marginBottom:4}}>{isRTL?"יחידה":"Unit"}</div><input value={form.unit} onChange={e=>setForm({...form,unit:e.target.value})} placeholder="kg" style={S.inp}/></div>
              <div><div style={{fontSize:11,color:TH.subText,marginBottom:4}}>{isRTL?"קטגוריה":"Category"}</div><select value={form.cat} onChange={e=>setForm({...form,cat:e.target.value})} style={{...S.inp,appearance:"none",color:TH.text}}>{cats.map(c=><option key={c.id} value={c.id} style={{background:TH.bg,color:TH.text}}>{c.icon} {c.label}</option>)}</select></div>
            </div>
            <div style={{marginBottom:12}}><div style={{fontSize:11,color:TH.subText,marginBottom:4}}>{isRTL?"הערה":"Note"}</div><input value={form.note} onChange={e=>setForm({...form,note:e.target.value})} style={S.inp}/></div>
            <button onClick={addItem} style={{width:"100%",padding:"13px",background:"linear-gradient(135deg,#10b981,#059669)",border:"none",borderRadius:12,color:"#fff",fontSize:15,fontWeight:700,cursor:"pointer"}}>+ {isRTL?"הוסף לרשימה":"Add to List"}</button>
          </div>
        )}
        <div style={{...S.card,marginBottom:16}}>
          <div style={{fontSize:12,fontWeight:700,color:"#a5b4fc",marginBottom:12}}>⚡ {isRTL?"הוספה מהירה":"Quick Add"}</div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8}}>
            {FAVS.map(fav=>{const avg=getAvg(fav.he);return(<button key={fav.id} onClick={()=>addFav(fav)} style={{background:favAdded[fav.id]?"rgba(16,185,129,0.15)":TH.input,border:favAdded[fav.id]?"1px solid rgba(16,185,129,0.3)":"1px solid "+TH.cardBorder,borderRadius:12,padding:"12px 8px",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:5}}><span style={{fontSize:24}}>{favAdded[fav.id]?"✓":fav.icon}</span><span style={{fontSize:12,color:favAdded[fav.id]?"#6ee7b7":TH.text,fontWeight:600}}>{isRTL?fav.he:fav.en}</span>{avg&&<span style={{fontSize:11,color:"#10b981",fontWeight:700}}>₪{avg}</span>}</button>);})}
          </div>
        </div>
        <div style={{display:"flex",gap:8,marginBottom:14,alignItems:"center"}}>
          ...
          {doneCount>0&&<button onClick={()=>setShowDone(!showDone)} style={S.pill(showDone)}>{showDone?(isRTL?"הסתר":"Hide"):(isRTL?"נקנו":"Bought")} ({doneCount})</button>}
          {showDone&&doneCount>0&&<button onClick={clearDone} style={{background:"rgba(239,68,68,0.1)",border:"1px solid rgba(239,68,68,0.2)",borderRadius:20,padding:"5px 12px",color:"#fca5a5",fontSize:12,cursor:"pointer",whiteSpace:"nowrap"}}>{isRTL?"נקה":"Clear"}</button>}
        </div>
        {cats.map(cat=>{
