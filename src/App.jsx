...
const INITIAL_TASKS = [
  {id:1,he:"לקבוע תור לרופא שיניים",en:"Book dentist appointment",person:"Raz",done:false,priority:"high"},
  {id:2,he:"לקנות מצרכים",en:"Buy groceries",person:"Olga",done:true,priority:"medium"},
  {id:3,he:"לשלם חשבון חשמל",en:"Pay electricity bill",person:"Raz",done:false,priority:"high"},
  {id:4,he:"לקבוע טיפול וטרינרי לקלי",en:"Schedule vet visit for Kelly",person:"Olga",done:false,priority:"medium"},
  {id:5,he:"תזכורת טיפול ברכב",en:"Car service reminder",person:"Raz",done:false,priority:"low"},
];
                ...
                <div style={card}>
                  ...
                  <div style={{display:"flex",flexDirection:"column",gap:8}}>
                    {tasks.map(task=>(<div key={task.id} onClick={()=>toggleTask(task.id)} style={{display:"flex",alignItems:"center",flexDirection:isRTL?"row-reverse":"row",gap:12,padding:"11px 14px",borderRadius:12,cursor:"pointer",background:task.done?"rgba(16,185,129,.06)":"rgba(255,255,255,.03)",border:task.done?"1px solid rgba(16,185,129,.15)":"1px solid rgba(255,255,255,.05)"}}>
                      ...
                      <div style={{flex:1,fontSize:14,fontWeight:500,textDecoration:task.done?"line-through":"none",color:task.done?"#4b5563":"#e8eaf0",textAlign:isRTL?"right":"left"}}>{task[lang]}</div>
                      <div style={{display:"flex",gap:6,flexShrink:0}}>
                        ...
                        <span style={{fontSize:10,padding:"2px 7px",borderRadius:20,background:task.person==="Raz"?"rgba(99,102,241,.15)":"rgba(6,182,212,.15)",color:task.person==="Raz"?"#a5b4fc":"#67e8f9",fontWeight:600}}>{task.person}</span>
                      </div>
                    </div>))}
                  </div>
                ...
                <div style={{display:"flex",flexDirection:"column",gap:20}}>
                  ...
                  <div style={{...card,flex:1}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16,flexDirection:isRTL?"row-reverse":"row"}}>
                      ...
                      {alerts.length>0&&<div style={{background:"rgba(239,68,68,.12)",border:"1px solid rgba(239,68,68,.25)",borderRadius:8,padding:"5px 10px",fontSize:11,color:"#fca5a5",whiteSpace:"nowrap"}}>{tr.health.refillAlert(alerts.length)}</div>}
                    </div>
                    <div style={{display:"flex",flexDirection:"column",gap:8}}>
                      {HEALTH_ITEMS.map(item=>{const urgent=item.daysLeft!==undefined&&item.daysLeft<=7;return(
                        <div key={item.id} style={{display:"flex",alignItems:"center",flexDirection:isRTL?"row-reverse":"row",gap:12,padding:"11px 14px",borderRadius:12,background:urgent?"rgba(239,68,68,.06)":"rgba(255,255,255,.03)",border:urgent?"1px solid rgba(239,68,68,.2)":"1px solid rgba(255,255,255,.05)"}}>
                          ...
                          <div style={{flex:1,textAlign:isRTL?"right":"left"}}><div style={{fontSize:13,fontWeight:600}}>{item[lang]}</div><div style={{fontSize:11,color:"#6b7280"}}>{item.person}</div></div>
                          <div style={{textAlign:isRTL?"left":"right",flexShrink:0}}>{item.daysLeft!==undefined?(<><div style={{fontSize:13,fontWeight:700,color:urgent?"#ef4444":"#9ca3af"}}>{tr.health.daysLeft(item.daysLeft)} {urgent&&"⚠"}</div>{urgent&&<div style={{fontSize:10,color:"#ef4444",marginTop:1}}>{tr.health.refillSoon}</div>}</>):(<div style={{fontSize:12,color:"#9ca3af"}}>{tr.health.due} {item.dueDate}</div>)}</div>
                        </div>
                      );})}
                    </div>
                  </div>
