...
// Next due date calculator for recurring tasks
...
const SAMPLE_TASKS = [
  ...
  { id:"t3",  he:"ועד בית",              en:"Building Committee",   cat:"bills",    priority:"medium", person:"Olga", recur:"monthly",   dueDate:"2026-04-10", lastDone:"2026-03-10", done:false, note:"העברה בנקאית 350 שח" },
  ...
  { id:"t5",  he:"טיפול רכב",            en:"Car Service",          cat:"car",      priority:"medium", person:"Raz",  recur:"quarterly", dueDate:"2026-04-20", lastDone:"2026-01-20", done:false, note:"כל 10,000 ק"מ" },
  ...
  { id:"t9",  he:"קנייה לבית",           en:"Grocery Shopping",     cat:"home",     priority:"medium", person:"Olga", recur:"weekly",    dueDate:"2026-03-23", lastDone:"2026-03-16", done:false, note:"" },
...
export default function Tasks({ lang }) {
  ...
  const T = {
    ...
    all:        isRTL ? "הכל" : "All",
    pending:    isRTL ? "ממתינות" : "Pending",
    done:       isRTL ? "הושלמו" : "Done",
    overdue:    isRTL ? "באיחור" : "Overdue",
    today:      isRTL ? "היום" : "Today",
    thisWeek:   isRTL ? "השבוע" : "This Week",
    recurring:  isRTL ? "מחזוריות" : "Recurring",
  ...
  const [view,     setView]     = useState("pending"); // pending | done | overdue | recurring
  ...
  const saveTask = () => {
    ...
    if (editId) {
      setTasks(p => p.map(t => t.id===editId ? {...form, id:editId, done:t.done, lastDone:t.lastDone} : t));
    } else {
      setTasks(p => [...p, {...form, id:newId(), he:form.he.trim(), en:form.en||form.he.trim(), done:false, lastDone:""}]);
    }
    setShowForm(false);
    setEditId(null);
  };
  ...
  const TaskCard = ({ t }) => {
    ...
    const pc   = PERSON_COLORS[t.person] || PERSON_COLORS["Both"];
    const isOverdue = d !== null && d < 0;

    return (
      <div style={{
        background: t.done ? "rgba(16,185,129,0.04)" : isOverdue ? "rgba(239,68,68,0.05)" : "rgba(255,255,255,0.03)",
        border: t.done ? "1px solid rgba(16,185,129,0.15)" : isOverdue ? "1px solid rgba(239,68,68,0.2)" : "1px solid rgba(255,255,255,0.07)",
        borderRadius:14, padding:"14px 16px", marginBottom:10,
        borderRight: isRTL && !t.done ? "3px solid "+(PRIORITY_COLORS[t.priority]||"#6b7280") : undefined,
        borderLeft:  !isRTL && !t.done ? "3px solid "+(PRIORITY_COLORS[t.priority]||"#6b7280") : undefined,
      }}>
        {/* Top row */}
        <div style={{display:"flex", alignItems:"flex-start", gap:10, flexDirection:isRTL?"row-reverse":"row"}}>
          {/* Checkbox */}
          <div onClick={()=>!t.done&&markDone(t.id)} style={{
            width:22, height:22, borderRadius:7, flexShrink:0, marginTop:1,
            background: t.done ? "#10b981" : "transparent",
            border: t.done ? "2px solid #10b981" : "2px solid rgba(255,255,255,0.2)",
            display:"flex", alignItems:"center", justifyContent:"center",
            fontSize:12, color:"#fff", cursor: t.done ? "default" : "pointer",
          }}>{t.done ? "✓" : ""}</div>
          ...
          <div style={{flex:1}}>
            <div style={{display:"flex", alignItems:"center", gap:8, flexDirection:isRTL?"row-reverse":"row", flexWrap:"wrap"}}>
              ...
              <span style={{
                ...
                color: t.done ? "#4b5563" : "#e8eaf0",
              }}>{isRTL ? t.he : t.en}</span>
        ...
        {!t.done && (
          <div style={{display:"flex", gap:6, marginTop:10, flexDirection:isRTL?"row-reverse":"row"}}>
            <button onClick={()=>markDone(t.id)} style={{
              ...
              borderRadius:8, padding:"5px 12px", color:"#6ee7b7", fontSize:12, fontWeight:600, cursor:"pointer",
            }}>✓ {T.markDone}</button>
            <button onClick={()=>openEdit(t)} style={{
              background:"rgba(99,102,241,0.1)", border:"1px solid rgba(99,102,241,0.2)",
              borderRadius:8, padding:"5px 12px", color:"#a5b4fc", fontSize:12, cursor:"pointer",
            }}>✏️</button>
            <button onClick={()=>deleteTask(t.id)} style={{
              background:"rgba(239,68,68,0.08)", border:"1px solid rgba(239,68,68,0.15)",
              borderRadius:8, padding:"5px 10px", color:"#fca5a5", fontSize:12, cursor:"pointer",
            }}>🗑</button>
          </div>
        )}
      </div>
  ...
  const TaskForm = () => (
    <div style={{...S.card, marginBottom:20, borderColor:"rgba(99,102,241,0.3)"}}>
      <div style={{fontSize:14, fontWeight:700, color:"#a5b4fc", marginBottom:14}}>
        {editId ? (isRTL?"עריכת משימה":"Edit Task") : (isRTL?"משימה חדשה":"New Task")}
      </div>
      {/* Row 1: name + cat */}
      <div style={{display:"grid", gridTemplateColumns:"2fr 1fr", gap:10, marginBottom:10}}>
        <div>
          <div style={{fontSize:11, color:"#6b7280", marginBottom:4}}>{T.taskName}</div>
          <input value={form.he} onChange={e=>setForm({...form,he:e.target.value})} placeholder={isRTL?"לדוגמה: תשלום ארנונה":"e.g. Pay Arnona"} style={S.inp} autoFocus onKeyDown={e=>e.key==="Enter"&&saveTask()}/>
        </div>
        <div>
          ...
          <select value={form.cat} onChange={e=>setForm({...form,cat:e.target.value})} style={{...S.inp,appearance:"none"}}>
            {CATS.map(c=><option key={c.id} value={c.id}>{c.icon} {isRTL?c.he:c.en}</option>)}
          </select>
        </div>
      </div>
      {/* Row 2: recur + dueDate + person + priority */}
      <div style={{display:"grid", gridTemplateColumns:"1fr 1fr 1fr 1fr", gap:10, marginBottom:10}}>
        <div>
          <div style={{fontSize:11, color:"#6b7280", marginBottom:4}}>{T.recurType}</div>
          <select value={form.recur} onChange={e=>setForm({...form,recur:e.target.value})} style={{...S.inp,appearance:"none"}}>
            {RECUR_OPTIONS.map(r=><option key={r.id} value={r.id}>{isRTL?r.he:r.en}</option>)}
          </select>
        </div>
        <div>
          <div style={{fontSize:11, color:"#6b7280", marginBottom:4}}>{T.dueDate}</div>
        ...
        <div>
          <div style={{fontSize:11, color:"#6b7280", marginBottom:4}}>{T.priority}</div>
          <select value={form.priority} onChange={e=>setForm({...form,priority:e.target.value})} style={{...S.inp,appearance:"none"}}>
            {["high","medium","low"].map(p=><option key={p} value={p}>{PRIORITY_LABELS[lang][p]}</option>)}
          </select>
        </div>
      </div>
      {/* Note */}
      <div style={{marginBottom:12}}>
        ...
        <input value={form.note} onChange={e=>setForm({...form,note:e.target.value})} placeholder={isRTL?"הערה אופציונלית...":"Optional note..."} style={S.inp}/>
      </div>
      {/* Buttons */}
    ...
    <div style={{fontFamily:"'Outfit',sans-serif", color:"#e8eaf0", direction:isRTL?"rtl":"ltr", minHeight:"100vh", background:"#0f1117"}}>
      ...
      <div style={{padding:"18px 22px", maxWidth:860, margin:"0 auto"}}>
        ...
        <div style={{display:"grid", gridTemplateColumns:"repeat(5,1fr)", gap:12, marginBottom:20}} className="tasks-stats">
          {[
            { label:isRTL?"באיחור":"Overdue",   val:overdueCount, color:"#ef4444", click:"overdue"   },
            { label:isRTL?"היום":"Today",        val:todayCount,   color:"#f59e0b", click:"pending"   },
            { label:isRTL?"השבוע":"This Week",   val:weekCount,    color:"#06b6d4", click:"pending"   },
            { label:isRTL?"מחזוריות":"Recurring",val:recurCount,   color:"#a855f7", click:"recurring" },
            { label:isRTL?"הושלמו":"Done",        val:doneCount,    color:"#10b981", click:"done"      },
          ].map(s=>(
            <div key={s.label} onClick={()=>setView(s.click)} style={{
              ...S.card, textAlign:"center", cursor:"pointer",
              borderTop:"2px solid "+s.color+"55",
              background: view===s.click ? "rgba(99,102,241,0.08)" : "rgba(255,255,255,0.03)",
            }}>
        ...
        <div style={{display:"flex", gap:8, marginBottom:14, flexWrap:"wrap", alignItems:"center"}}>
          ...
          <div style={{position:"relative", flex:"1 1 150px"}}>
            ...
            <input value={search} onChange={e=>setSearch(e.target.value)} placeholder={isRTL?"חיפוש משימה...":"Search task..."}
          ...
          <div style={{display:"flex",gap:3,background:"rgba(255,255,255,0.04)",borderRadius:10,padding:3}}>
            {[
              ...
              {v:"overdue",  l:isRTL?"באיחור":"Overdue"},
              ...
              }}>{l}{v==="overdue"&&overdueCount>0&&<span style={{background:"#ef4444",color:"#fff",fontSize:9,fontWeight:700,borderRadius:20,padding:"1px 5px",marginRight:4}}>{overdueCount}</span>}</button>
            ))}
          </div>
          {/* Person filter */}
        ...
        <div style={{display:"flex",gap:5,marginBottom:16,overflowX:"auto",paddingBottom:3}}>
          ...
          {CATS.map(c=>(
            <button key={c.id} onClick={()=>setFilterC(c.id)} style={S.pill(filterC===c.id)}>{c.icon} {isRTL?c.he:c.en}</button>
          ))}
        </div>
