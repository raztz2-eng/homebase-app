...
// ── TaskForm — standalone to avoid focus issues ────────────
function TaskForm({form,setForm,editId,save,cancel,lang}){
  const isRTL=lang==="he";
  // Use refs to track inputs — prevents re-render focus loss
  ...
  return(
    <div style={{...CARD,marginBottom:20,borderColor:"rgba(99,102,241,0.3)"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"2fr 1fr",gap:10,marginBottom:10}}>
        <div>
          <div style={{fontSize:11,color:"#6b7280",marginBottom:4}}>{isRTL?"שם המשימה":"Task name"}</div>
          <input
            value={form.he}
            onChange={e=>handleChange("he",e.target.value)}
            placeholder={isRTL?"לדוגמה: תשלום ארנונה":"e.g. Pay Arnona"}
            style={INP}
            autoFocus
          />
        </div>
        <div>
          <div style={{fontSize:11,color:"#6b7280",marginBottom:4}}>{isRTL?"קטגוריה":"Category"}</div>
          <select value={form.cat} onChange={e=>handleChange("cat",e.target.value)} style={SEL}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:10,marginBottom:10}}>
        <div>
          <div style={{fontSize:11,color:"#6b7280",marginBottom:4}}>{isRTL?"תדירות":"Frequency"}</div>
          <select value={form.recur} onChange={e=>handleChange("recur",e.target.value)} style={SEL}>
            {RECUR.map(r=><option key={r.id} value={r.id} style={{background:"#1f2937",color:"#e8eaf0"}}>{isRTL?r.he:r.en}</option>)}
          </select>
        </div>
        <div>
          <div style={{fontSize:11,color:"#6b7280",marginBottom:4}}>{isRTL?"תאריך יעד":"Due date"}</div>
          <input type="date" value={form.dueDate} onChange={e=>handleChange("dueDate",e.target.value)} style={INP}/>
        </div>
        <div>
          <div style={{fontSize:11,color:"#6b7280",marginBottom:4}}>{isRTL?"אחראי":"Assignee"}</div>
          <select value={form.person} onChange={e=>handleChange("person",e.target.value)} style={SEL}>
            {["Raz","Olga",isRTL?"שניהם":"Both"].map(p=><option key={p} style={{background:"#1f2937",color:"#e8eaf0"}}>{p}</option>)}
          </select>
        </div>
        <div>
          <div style={{fontSize:11,color:"#6b7280",marginBottom:4}}>{isRTL?"עדיפות":"Priority"}</div>
          <select value={form.priority} onChange={e=>handleChange("priority",e.target.value)} style={SEL}>
            {["high","medium","low"].map(p=><option key={p} value={p} style={{background:"#1f2937",color:"#e8eaf0"}}>{PL[lang][p]}</option>)}
          </select>
        </div>
      </div>
      <div style={{marginBottom:12}}>
        <div style={{fontSize:11,color:"#6b7280",marginBottom:4}}>{isRTL?"הערה":"Note"}</div>
        <input
          ref={noteRef}
          value={form.note}
      ...
      <div style={{display:"flex",gap:8,flexDirection:isRTL?"row-reverse":"row"}}>
        ...
        <button onClick={cancel} style={{...BTN,background:"rgba(255,255,255,0.08)"}}>{isRTL?"ביטול":"Cancel"}</button>
      </div>
...
export default function Tasks({lang,onNavigate}){
    ...
    return(
      <div style={{background:t.done?"rgba(16,185,129,0.04)":ov?"rgba(239,68,68,0.05)":"rgba(255,255,255,0.03)",border:t.done?"1px solid rgba(16,185,129,0.15)":ov?"1px solid rgba(239,68,68,0.2)":"1px solid rgba(255,255,255,0.07)",borderRadius:14,padding:"14px 16px",marginBottom:10,[isRTL?"borderRight":"borderLeft"]:"3px solid "+(t.done?"transparent":(PC[t.priority]||"#6b7280"))}}>
        <div style={{display:"flex",alignItems:"flex-start",gap:10,flexDirection:isRTL?"row-reverse":"row"}}>
          ...
          {t.dueDate&&!t.done&&<div style={{textAlign:"center",flexShrink:0}}><div style={{fontSize:12,fontWeight:700,color:u||"#6b7280"}}>{du(t.dueDate)===0?(isRTL?"היום!":"Today!"):du(t.dueDate)<0?(isRTL?Math.abs(du(t.dueDate))+" ימי איחור":Math.abs(du(t.dueDate))+" overdue"):(isRTL?"עוד "+du(t.dueDate)+" ימים":"in "+du(t.dueDate)+" days")}</div><div style={{fontSize:10,color:"#6b7280"}}>{t.dueDate}</div></div>}
          {t.done&&t.recur!=="none"&&t.dueDate&&<div style={{textAlign:"center",flexShrink:0,fontSize:11,color:"#6b7280"}}><div>{isRTL?"הבא:":"Next:"}</div><div style={{fontWeight:700,color:"#06b6d4"}}>{t.dueDate}</div></div>}
        </div>
        {!t.done&&(
  ...
  if(loading)return(<div style={{fontFamily:"'Outfit',sans-serif",color:"#e8eaf0",background:"#0f1117",minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",gap:14,fontSize:16}}><div style={{width:32,height:32,border:"3px solid rgba(99,102,241,0.3)",borderTop:"3px solid #6366f1",borderRadius:"50%",animation:"spin 1s linear infinite"}}/>{isRTL?"מסנכרן...":"Syncing..."}<style>{"@keyframes spin{to{transform:rotate(360deg)}}"}</style></div>);

  return(
    <div style={{fontFamily:"'Outfit',sans-serif",color:"#e8eaf0",direction:isRTL?"rtl":"ltr",minHeight:"100vh",background:"#0f1117"}}>
      <Confetti show={confetti}/>
      <div style={{background:"rgba(17,19,30,0.95)",borderBottom:"1px solid rgba(255,255,255,0.07)",padding:"12px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,flexWrap:"wrap"}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <div>
            <div style={{fontSize:17,fontWeight:800}}>{isRTL?"משימות":"Tasks"}</div>
        ...
        <div style={{display:"flex",gap:8,alignItems:"center"}}>
          <button onClick={()=>onNavigate&&onNavigate("dashboard")} style={{background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.12)",borderRadius:10,padding:"6px 12px",color:"#9ca3af",fontSize:12,cursor:"pointer"}}>⬅ {isRTL?"לוח בקרה":"Dashboard"}</button>
          <button onClick={openAdd} style={BTN}>+ {isRTL?"משימה":"Task"}</button>
        </div>
      </div>
      <div style={{padding:"14px 16px",maxWidth:860,margin:"0 auto"}}>
        <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:10,marginBottom:16}} className="tasks-stats">
          {[{l:isRTL?"באיחור":"Overdue",v:oc,c:"#ef4444",k:"overdue"},{l:isRTL?"היום":"Today",v:tc,c:"#f59e0b",k:"pending"},{l:isRTL?"השבוע":"Week",v:wc,c:"#06b6d4",k:"pending"},{l:isRTL?"מחזוריות":"Recurring",v:rc,c:"#a855f7",k:"recurring"},{l:isRTL?"הושלמו":"Done",v:dc,c:"#10b981",k:"done"}].map(s=>(
            <div key={s.l} onClick={()=>setView(s.k)} style={{...CARD,textAlign:"center",cursor:"pointer",borderTop:"2px solid "+s.c+"55",padding:"12px 8px",background:view===s.k?"rgba(99,102,241,0.08)":"rgba(255,255,255,0.03)"}}>
              <div style={{fontSize:20,fontWeight:800,color:s.c}}>{s.v}</div>
              <div style={{fontSize:10,color:"#6b7280",marginTop:2}}>{s.l}</div>
            </div>
          ))}
        </div>
        {showForm&&<TaskForm form={form} setForm={setForm} editId={editId} save={save} cancel={()=>{setShowForm(false);setEditId(null);}} lang={lang}/>}
        <div style={{display:"flex",gap:6,marginBottom:12,flexWrap:"wrap",alignItems:"center"}}>
          <div style={{position:"relative",flex:"1 1 140px",minWidth:140}}>
            ...
            <input value={q} onChange={e=>setQ(e.target.value)} placeholder={isRTL?"חיפוש...":"Search..."} style={{...INP,[isRTL?"paddingRight":"paddingLeft"]:32}}/>
          </div>
          <div style={{display:"flex",gap:3,background:"rgba(255,255,255,0.04)",borderRadius:10,padding:3}}>
            {[{v:"pending",l:isRTL?"ממתינות":"Pending"},{v:"overdue",l:isRTL?"באיחור":"Overdue"},{v:"recurring",l:isRTL?"מחזוריות":"Recur"}].map(({v,l})=>(
              <button key={v} onClick={()=>setView(v)} style={{background:view===v?"rgba(99,102,241,0.3)":"transparent",border:"none",borderRadius:8,padding:"5px 10px",color:view===v?"#a5b4fc":"#6b7280",fontSize:12,fontWeight:600,cursor:"pointer"}}>
                {l}{v==="overdue"&&oc>0&&<span style={{background:"#ef4444",color:"#fff",fontSize:9,borderRadius:20,padding:"1px 5px",marginRight:4}}>{oc}</span>}
              </button>
            ))}
          </div>
          <button onClick={()=>setShowDone(!showDone)} style={{background:showDone?"rgba(16,185,129,0.15)":"rgba(255,255,255,0.04)",border:showDone?"1px solid rgba(16,185,129,0.3)":"1px solid rgba(255,255,255,0.08)",borderRadius:20,padding:"5px 12px",color:showDone?"#6ee7b7":"#6b7280",fontSize:12,fontWeight:600,cursor:"pointer"}}>
        ...
        {filtered.length===0?(<div style={{...CARD,textAlign:"center",padding:40,color:"#6b7280"}}><div style={{fontSize:36,marginBottom:10}}>🎉</div>{isRTL?"אין משימות":"No tasks"}</div>):(filtered.map(t=><Card key={t.id} t={t}/>))}
      </div>
